...
| **Question ↔ GlobalSummary** | 1:N   | 한 문제는 여러 통합 요약(GlobalSummary)와 연결 | 문제별 1개 요약 권장     |
...
| 규칙 ID       | 규칙 내용                   | 우선순위 | 검증 방법                |
|-------------|-------------------------|------|----------------------|
| **BR-U001** | 이메일은 회원별로 유일해야 함        | 높음   | 회원가입/수정 시 중복 검사      |
...
| **BR-R004** | Q004	문제 풀이 마감일 이후에는 풀이 불가     | 높음   | 문제 조회/풀이 시 날짜 비교  |
...
| **UserService**              | 홈 관리                                 | 홈 페이지에 필요한 값 일괄 요청, 조별 점수 계산                    |
...
- [ ] 사원번호는 관리자가 시스템에 미리 등록하며, 중복되지 않음
- [ ] 회원들은 기본적인 정보 보안에 대한 지식이 있음
- [ ] 문제는 사전에 등록되어 있음
- [ ] AI 답변 생성 및 요약 기능은 외부 AI 서비스와 연동하여 제공됨
- [ ] 시스템은 한국어로만 서비스 제공

### 8.2 제약조건 (Constraints)
- [ ] **기술적 제약**: Java 17, Spring Boot 3.4, MariaDB 10.11 사용
- [ ] **성능 제약**: 응답 시간 3초 이내, 동시 사용자 100명
- [ ] **보안 제약**: 개인정보보호법 준수, HTTPS 필수, 비밀번호 암호화 저장
